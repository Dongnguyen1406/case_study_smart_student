...
public class StudentRepository implements IStudentRepository {
    ...
    private final String SELECT_STUDENT_CLASS = "SELECT s.student_id, s.student_name, s.gender, s.dob, s.address, s.email, s.number_phone, c.class_id FROM students s LEFT JOIN classes c ON c.class_id = s.class_id WHERE s.is_delete = 0;";
    ...
    private final String UPDATE_STUDENT_S =
            "UPDATE students SET student_name=?, dob=?, gender=?, address=?, number_phone=?, email=? WHERE student_id=?";
    ...
    public List<StudentDto> findAll() {
        ...
        try (Connection connection = BaseRepository.getConnectDB();
            ...
            while (resultSet.next()) {
                ...
                studentDtos.add(new StudentDto(studentId, studentName, dob, gender, address, numberPhone, email, className, classId));
    ...
    public boolean update(StudentDto studentDto) {
        try (Connection connection = BaseRepository.getConnectDB();
            ...
            preparedStatement.setDate(2, java.sql.Date.valueOf(studentDto.getDob()));
            preparedStatement.setString(3, studentDto.getGender());
            preparedStatement.setString(4, studentDto.getAddress());
            preparedStatement.setString(5, studentDto.getNumberPhone());
            preparedStatement.setString(6, studentDto.getEmail());
            preparedStatement.setString(7, studentDto.getStudentId());
            preparedStatement.setInt(8, studentDto.getClassId());
